...
float printPixel[width][height][3];

scene_t scene; //need global variable for scene for create function

void * create(void* threads)
{
    Vec3 camera_pos;
    ...
    : pixel_dy;
    /* for every pixel */
    for( int px = *(int*)threads; px<width; px += nthreads )
    {
        ...
        for( int py=0; py<height; ++py )
        {
             ...
             */
            for(int i = 0; i < 3; i++){
                printPixel[px][py][i] = scaled_color[i];
            }
        }
    }
    return NULL;
...
main( int argc, char **argv )
{
    ...
    {
        if(pthread_create(&threads[i], NULL, create, &t_id[i]) < 0)
        {
    ...
    for(int i = 0; i < width; i++)
    {
        ...
        {
            printf("%.0f %.0f %.0f\n",
                   printPixel[i][j][0], printPixel[i][j][1], printPixel[i][j][2]);
        }
        printf("\n");
    }
